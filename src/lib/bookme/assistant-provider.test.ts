...
async function main() {
  ...
  try {
    ...
    assert.equal(needsConfirmFor({ type: "draft_swap", lessonAId: "l1", lessonBId: "l2", note: "x" }), true);
    assert.equal(needsConfirmFor({ type: "cancel_lesson", lessonId: "l1" }), true);
    assert.equal(needsConfirmFor({ type: "list_lessons" }), false);
    assert.deepEqual(coerceAssistantAction({ type: "cancel_lesson", lessonId: "l2" }, caps), {
      type: "cancel_lesson",
      lessonId: "l2",
    });
    assert.equal(
      coerceAssistantAction({ type: "draft_swap", lessonAId: "l1", lessonBId: "l1", note: "x" }, caps),
      null,
    );
    assert.deepEqual(
