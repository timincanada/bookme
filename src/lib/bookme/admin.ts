...
/** Admin = the founder email AND a verified address (prevents claiming it by sign-up). */
export function isVerifiedAdmin(user: { email?: string | null; emailVerified?: boolean | null } | null | undefined) {
  return Boolean(user && user.emailVerified === true && isAdminEmail(user.email));
}
...
export function isStaffEmail(email?: string | null, staffEmails: string[] = [ADMIN_EMAIL]) {
  const needle = normalizeAdminEmail(email);
  if (!needle) return false;
  return staffEmails.some((e) => normalizeAdminEmail(e) === needle);
}

export function visibleCoaches<T extends { email: string }>(
  coaches: T[],
  staffEmails: string[] = [ADMIN_EMAIL],
) {
