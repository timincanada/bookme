...
export const auth = betterAuth({
  ...
  ...(emailAndPasswordEnabled
    ? {
        emailAndPassword: {
          ...
          // Sign-in only after the address is confirmed (stops claiming someone
          // else's email, including the admin address).
        ...
        emailVerification: {
          ...
          sendVerificationEmail: async ({ user, url }: { user: { email: string; name?: string | null }; url: string }) => {
            const { sendMail } = await import("@/lib/bookme/mail");
            await sendMail(
              {
                to: user.email,
                subject: "Confirm your BookMe email",
                text: `Hi ${user.name || "there"},\n\nConfirm your email to finish setting up BookMe:\n${url}\n\nIf you didn't sign up, ignore this email.`,
              },
  ...
  // Brute-force protection for sign-in / sign-up (stored in Postgres so it holds
