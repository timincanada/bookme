...
type Tab = { to: "/admin" | "/admin/coaches" | "/admin/revenue" | "/admin/lessons" | "/admin/lookup" | "/admin/health" | "/admin/team"; label: string; exact?: boolean; ownerOnly?: boolean };
...
function AdminShell() {
  ...
  const [state, setState] = useState<{ role: string; email: string } | null | "denied">(null);

  useEffect(() => {
    if (!user) return;
    adminSummary({ data: { period: "today" } }).then((r) => setState(r.ok ? { role: r.role, email: r.email } : "denied"));
  }, [user]);
