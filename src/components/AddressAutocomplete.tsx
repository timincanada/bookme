  ...
  return (
    <div ref={wrapRef} className="relative">
      <input
        ...
        autoCorrect="off"
        spellCheck={false}
        placeholder={disabled ? undefined : "Start typing an address"}
        onChange={(e) => {
      ...
      />
      {configured === false && (
        <p className="mt-1 text-sm text-muted">Address search unavailable</p>
      )}
      ...
      {configured && value.verified && (
        <p className="mt-1 text-sm text-brand-dark">Verified place</p>
      )}
        ...
        <ul className="absolute z-20 mt-1 max-h-64 w-full overflow-auto rounded-xl border border-line bg-surface shadow-lg">
          {suggestions.map((s) => (
            <li key={s.placeId}>
              <button
                type="button"
                className="flex w-full items-start justify-between gap-2 border-b border-line px-3 py-2.5 text-left last:border-0 hover:bg-brand-soft"
                onClick={() => pick(s)}
              >
                ...
                <span className="mt-1 h-4 w-4 shrink-0 rounded-full border-2 border-brand" aria-hidden />
              </button>
            </li>
          ))}
        </ul>
