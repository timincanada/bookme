...
export function PayChip({ kind, text }: { kind: string; text: string }) {
  return (
    <span
      className={cn(
        "type-badge rounded-full px-2.5 py-0.5 text-xs font-medium",
...
export function StatusChip({ children }: { children: string }) {
  return <span className="type-badge rounded-full border border-line px-2.5 py-0.5 text-xs font-medium text-muted">{children}</span>;
}
